...
--Müþterilerin sipariþ sayýlarýný, sipariþ ettikleri ürün sayýlarýný ve 
--ürünlere ödedikleri toplam net miktarý raporlayýnýz.
...
--Þehirlerde verilen sipariþ sayýlarýný, sipariþ edilen ürün sayýlarýný ve 
--ürünlere ödenen toplam net miktarlarý raporlayýnýz.
...
SELECT	a.state,
		COUNT (DISTINCT C.order_id) AS cnt_order,
		SUM(B.quantity) cnt_product,
		SUM(quantity*list_price*(1-discount))/count(distinct A.city) net_amount
FROM	sale.customer A, sale.order_item B, sale.orders C
WHERE	A.customer_id = C.customer_id
AND		B.order_id = C.order_id
GROUP BY  a.state
...
SELECT	DISTINCT A.state, A.city,
		AVG(quantity*list_price*(1-discount)) OVER (PARTITION BY A.state) avg_turnover_state,
		AVG(quantity*list_price*(1-discount)) OVER (PARTITION BY A.state,A.city) avg_turnover_city
...
SELECT *
FROM T1
WHERE	avg_turnover_city < avg_turnover_state

--Create a report shows daywise turnovers of the BFLO Store.
--BFLO Store Maðazasýnýn haftanýn günlerine göre elde ettiði
--ciro miktarýný gösteren bir rapor hazýrlayýnýz.
